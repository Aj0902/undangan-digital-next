...
export default function ClientForm({
  ...
  const handleTemplateChange = async (val: string) => {
    if (templateType === "dinamis" && val) {
      if (
        confirm(
          "Menerapkan preset ini akan mengganti desain kustom yang ada (jika ada). Lanjutkan?",
        )
      ) {
        startUpdate(async () => {
  ...
  const handleDelete = () => {
    if (
      confirm("Apakah Anda yakin ingin menghapus klien ini secara permanen?")
    ) {
      startDelete(async () => {
  ...
  const handleMediaUpload = async (
    ...
    setUploadErrors((prev) => ({ ...prev, [mediaKey]: "" })); // Clear previous errors
    ...
    if (result.error) {
      // ✅ Show error inline AND in toast
      setUploadErrors((prev) => ({ ...prev, [mediaKey]: result.error }));
      toast.error(result.error);
    } else {
      // ✅ Clear error dan show success
      setUploadErrors((prev) => ({ ...prev, [mediaKey]: "" }));
  ...
  const getMediaUrl = (key: string) => {
    return client.client_media?.find((m: any) => m.media_key === key)
      ?.cloudinary_url;
  };
    ...
    <div className="space-y-12 relative pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-white/5 pb-8">
        <div className="flex items-center gap-5">
          <Link
            href="/admin"
            className="p-3 bg-[#111] border border-white/10 rounded-2xl hover:bg-white/5 transition-all text-white/50 hover:text-amber-400 shadow-sm relative group overflow-hidden"
          >
          ...
          <div>
            <div className="flex items-center gap-4">
              <h1 className="text-3xl font-serif text-white tracking-tight uppercase">
                Client{" "}
                <span className="font-sans italic text-amber-500">Details</span>
              </h1>
              <span
                className={`px-4 py-1.5 rounded-xl text-[9px] font-bold uppercase tracking-[0.2em] ${client.is_active ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 shadow-[0_0_10px_rgba(52,211,153,0.1)]" : "bg-red-500/10 text-red-400 border border-red-500/20"}`}
              >
            ...
            <p className="text-white/40 font-bold uppercase tracking-widest text-[10px] mt-2 flex items-center gap-2">
              Slug Index:{" "}
              <span className="text-amber-500 font-bold tracking-normal italic ml-1">
                /{client.slug}
              </span>
              <Link
                href={`/${client.slug}`}
                target="_blank"
                className="p-1.5 hover:bg-white/5 rounded-lg transition-all text-white/30 hover:text-amber-400 border border-transparent hover:border-white/10"
              >
        ...
        <div className="flex items-center gap-3">
          {templateType === "dinamis" && (
            <Button
              asChild
              className="bg-emerald-500 hover:bg-emerald-400 text-black font-bold h-auto py-3.5 px-8 rounded-2xl shadow-xl shadow-emerald-900/20 uppercase tracking-[0.2em] text-[10px] transition-all transform hover:-translate-y-1"
            >
          ...
          <Button
            ...
            disabled={isDeleting}
            variant="destructive"
            className="rounded-2xl border border-red-500/20 uppercase tracking-widest text-[10px] font-bold h-auto py-3.5 px-6"
          >
            {isDeleting ? (
        ...
        <div className="lg:col-span-2 space-y-10">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-10">
            ...
            <Card className="bg-[#111]/80 backdrop-blur-xl rounded-[2.5rem] border border-white/10 shadow-2xl p-8 space-y-6 relative overflow-hidden">
              ...
              <CardHeader className="p-0 space-y-0">
                <div className="flex items-center gap-4 border-b border-white/5 pb-6">
                  ...
                  <CardTitle className="text-xl font-bold text-white uppercase tracking-widest">
                    1. Blueprint Config
                  </CardTitle>
                </div>
              </CardHeader>
              ...
              <CardContent className="p-0 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                  <div className="space-y-3">
                    ...
                    <Input
                      {...register("slug")}
                      className="bg-black/50 border-white/10 rounded-2xl h-14"
                    />
                    {errors.slug && (
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <label
                          className={`flex items-center justify-center gap-3 p-4 rounded-xl border transition-all cursor-pointer ${templateType === "statis" ? "bg-emerald-500/10 border-emerald-500 text-emerald-400" : "bg-white/5 border-white/5 text-white/40 hover:bg-white/10"}`}
                        >
                          <input
                            type="radio"
                            value="statis"
                        ...
                        <label
                          className={`flex items-center justify-center gap-3 p-4 rounded-xl border transition-all cursor-pointer ${templateType === "dinamis" ? "bg-emerald-500/10 border-emerald-500 text-emerald-400" : "bg-white/5 border-white/5 text-white/40 hover:bg-white/10"}`}
                        >
                          <input
                            type="radio"
                            value="dinamis"
                  ...
                  <div className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => {
                        ...
                        const event = {
                          target: { name: "is_active", value: val },
                        };
                    ...
                    <Label
                      htmlFor="is_active"
                      className="text-sm font-bold text-white/70 tracking-wide cursor-pointer"
                    >
                      Publish Immediately (Active State)
                    </Label>
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent className="p-0 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-4 p-8 bg-rose-950/20 rounded-3xl border border-rose-500/20">
                    <h3 className="text-[10px] font-extrabold text-rose-400 uppercase tracking-[0.3em]">
                      The Bride
                    </h3>
                    <Input
                      {...register("bride_name")}
                  ...
                  <div className="space-y-4 p-8 bg-indigo-950/20 rounded-3xl border border-indigo-500/20">
                    <h3 className="text-[10px] font-extrabold text-indigo-400 uppercase tracking-[0.3em]">
                      The Groom
                    </h3>
                    <Input
                      {...register("groom_name")}
                    ...
                    />
                    <Input
                      {...register("groom_full_name")}
                      placeholder="Full Legal Name"
                      className="bg-black/50 border-white/10"
                    />
                    <Input
                      {...register("groom_parents")}
                      placeholder="Parents Name"
                      className="bg-black/50 border-white/10"
                    />
                  </div>
                ...
                <div className="space-y-3 mt-6">
                  <Label className="text-[10px] font-extrabold text-white/40 uppercase tracking-[0.2em] px-1">
                    Prologue Text
                  </Label>
                  <Textarea
                    {...register("prologue_text")}
                    rows={3}
                    placeholder="Write something beautiful..."
                    className="bg-black/50 border-white/10 rounded-2xl resize-none"
                  />
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-0 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-4 p-8 bg-[#0a0a0a]/50 rounded-3xl border border-white/5">
                    <h3 className="text-[10px] font-extrabold text-emerald-400 uppercase tracking-[0.3em]">
                      Akad Nikah
                    </h3>
                    <Input
                      type="datetime-local"
                      {...register("akad_datetime")}
                      className="bg-black/50 border-white/10 [&::-webkit-calendar-picker-indicator]:filter [&::-webkit-calendar-picker-indicator]:invert"
                    />
                    <Input
                      {...register("akad_venue_name")}
                      placeholder="Venue Name"
                      className="bg-black/50 border-white/10"
                    />
                    <Textarea
                      {...register("akad_venue_address")}
                      placeholder="Full Address"
                      rows={2}
                      className="bg-black/50 border-white/10 rounded-xl resize-none"
                    />
                  </div>

                  {/* Resepsi */}
                  <div className="space-y-4 p-8 bg-[#0a0a0a]/50 rounded-3xl border border-white/5">
                    <h3 className="text-[10px] font-extrabold text-amber-500 uppercase tracking-[0.3em]">
                      Resepsi
                    </h3>
                    <Input
                      type="datetime-local"
                      {...register("resepsi_datetime")}
                      className="bg-black/50 border-white/10 [&::-webkit-calendar-picker-indicator]:filter [&::-webkit-calendar-picker-indicator]:invert"
                    />
                    <Input
                      {...register("resepsi_venue_name")}
                      placeholder="Venue Name"
                      className="bg-black/50 border-white/10"
                    />
                    <Textarea
                      {...register("resepsi_venue_address")}
                      placeholder="Full Address"
                      rows={2}
                      className="bg-black/50 border-white/10 rounded-xl resize-none"
                    />
            ...
            <Card className="bg-[#111]/80 backdrop-blur-xl rounded-[2.5rem] border border-white/10 shadow-2xl p-8 space-y-6">
              <CardHeader className="p-0">
                <div className="flex items-center justify-between border-b border-white/5 pb-6">
                  ...
                  <Button
                    ...
                    variant="outline"
                    onClick={() =>
                      appendBank({ bank: "", name: "", number: "" })
                    }
                    className="bg-amber-500/10 border-amber-500/20 text-amber-500 hover:bg-amber-500/20 hover:text-amber-400 font-bold text-[10px] uppercase tracking-widest"
                  >
              ...
              <CardContent className="p-0 space-y-4">
                {bankFields.map((field, index) => (
                  <div
                    key={field.id}
                    className="flex gap-4 items-start p-6 bg-[#0a0a0a]/50 rounded-3xl border border-white/5 relative group"
                  >
                    <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
                      <Input
                        {...register(`bank_accounts.${index}.bank` as const)}
                        placeholder="Bank (ex: BCA)"
                        required
                        className="bg-black/50 border-white/10"
                      />
                      <Input
                        {...register(`bank_accounts.${index}.number` as const)}
                        placeholder="Account No"
                        required
                        className="bg-black/50 border-white/10 font-mono"
                      />
                      <Input
                        {...register(`bank_accounts.${index}.name` as const)}
                        placeholder="A.N Name"
                        required
                        className="bg-black/50 border-white/10"
                      />
                    </div>
                    <Button
                      ...
                      className="text-white/20 hover:text-red-400 hover:bg-red-500/10 transition-all"
                    >
            ...
            <Button
              type="submit"
              disabled={isUpdating}
              className="w-full h-20 bg-amber-500 hover:bg-amber-400 text-black text-lg font-extrabold tracking-widest uppercase rounded-[2rem] transition-all duration-300 shadow-[0_0_20px_rgba(245,158,11,0.2)] hover:shadow-[0_0_30px_rgba(245,158,11,0.4)] flex items-center justify-center gap-3 transform hover:-translate-y-1 active:translate-y-0"
            >
              {isUpdating ? (
            ...
            <CardContent className="p-0 space-y-8 h-full">
              <div className="p-4 bg-amber-500/5 border border-amber-500/10 rounded-2xl">
                <p className="text-[10px] text-amber-500/70 font-bold leading-relaxed uppercase tracking-wider">
                  Note: Files upload instantly to cloud storage. Use portrait
                  ratios for High-Quality Covers.
                </p>
              </div>
                    ...
                    <div key={key} className="group relative">
                      <div className="flex items-center justify-between mb-3 px-1">
                        <Label className="text-[10px] font-bold text-white/50 uppercase tracking-[0.2em] flex items-center gap-2">
                          {isMusic ? (
                            <Music className="w-3.5 h-3.5 text-amber-500/50" />
                          ) : (
                            <ImageIcon className="w-3.5 h-3.5 text-amber-500/50" />
                          )}
                          {key.replace("_", " ")}
                        </Label>
                      ...
                      <div
                        className={`relative ${isMusic ? "h-[70px]" : "aspect-video"} rounded-3xl bg-black/50 overflow-hidden border border-dashed ${isUploading === key ? "border-amber-500" : hasError ? "border-red-500" : "border-white/20"} transition-all`}
                      >
                        {isUploading === key ? (
                          ...
                          <>
                            {isMusic ? (
                              <div className="flex items-center h-full px-6 gap-4">
                                ...
                                <span className="text-[10px] font-bold text-white/50 truncate uppercase tracking-widest">
                                  Audio Active
                                </span>
                              </div>
                            ) : (
                          ...
                          <label className="absolute inset-0 flex flex-col items-center justify-center cursor-pointer hover:bg-white/5 transition-all group-hover:border-amber-500/50">
                            ...
                            <span className="text-[10px] uppercase tracking-[0.3em] font-bold text-white/30 mt-4 group-hover:text-amber-500/70 transition-colors">
                              Upload {key}
                            </span>
                            <input
                              type="file"
                      ...
                      {hasError && (
                        <p className="text-[10px] font-bold text-red-500 mt-2 px-1 leading-relaxed">
                          {hasError}
                        </p>
                      )}
                    </div>
