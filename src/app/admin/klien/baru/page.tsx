...
import { ArrowLeft, Save, Info, Calendar, Heart, Settings, PlusCircle, MinusCircle, Loader2 } from 'lucide-react'
    ...
    <div className="space-y-10 max-w-4xl mx-auto pb-20 relative">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-5">
          <Link href="/admin" className="p-3 bg-[#111] border border-white/10 rounded-2xl hover:bg-white/5 transition-all text-white/50 hover:text-amber-400 shadow-sm">
            <ArrowLeft className="w-5 h-5" />
          </Link>
          <div>
            <h1 className="text-3xl font-serif text-white tracking-tight uppercase">New <span className="font-sans italic text-amber-500">Client</span></h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <section className="bg-[#111]/80 backdrop-blur-xl rounded-[2.5rem] border border-white/10 shadow-2xl p-8 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-3">
              <label className="text-[10px] font-bold text-white/50 block uppercase tracking-widest leading-none mb-1">
                URL Slug <span className="text-amber-500/50 italic">(ex: siti-zaed)</span>
              </label>
              <div className="relative group">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-white/30 font-medium text-sm group-focus-within:text-amber-500 transition-colors">/</span>
                <input 
                  {...register('slug')}
                  onChange={handleSlugUpdate}
                  placeholder="client-name"
                  className="w-full pl-8 pr-6 py-4 bg-black/50 border border-white/10 rounded-2xl focus:ring-1 focus:ring-amber-500 focus:border-amber-500 transition-all outline-none text-white font-bold text-sm"
                />
              </div>
            ...
            <div className="space-y-3">
              ...
              <select 
                ...
                className="w-full px-6 py-4 bg-black/50 border border-white/10 rounded-2xl focus:ring-1 focus:ring-amber-500 focus:border-amber-500 transition-all outline-none text-white font-bold cursor-pointer text-sm"
              >
          ...
          <div className="flex items-center gap-3 p-5 bg-black/30 rounded-2xl border border-white/5 group transition-colors">
        ...
        <section className="bg-[#111]/80 backdrop-blur-xl rounded-[2.5rem] border border-white/10 shadow-2xl p-8 space-y-6">
          <div className="flex items-center justify-between border-b border-white/5 pb-6">
            ...
            <button type="button" onClick={() => appendBank({ bank: '', name: '', number: '' })} className="px-5 py-2.5 bg-amber-500/10 border border-amber-500/20 text-amber-500 rounded-xl font-bold text-sm hover:bg-amber-500/20 transition-all flex items-center gap-2">
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 pt-10">
          <button 
            type="submit" 
            disabled={isPending}
            className="w-full md:flex-1 h-[70px] bg-amber-500 hover:bg-amber-400 text-black text-lg font-extrabold tracking-widest uppercase rounded-2xl transition-all duration-300 shadow-[0_0_20px_rgba(245,158,11,0.2)] hover:shadow-[0_0_30px_rgba(245,158,11,0.4)] flex items-center justify-center gap-3 transform hover:-translate-y-1 active:translate-y-0"
          >
            {isPending ? (
          ...
          <Link 
            href="/admin"
            className="w-full md:w-auto h-[70px] px-12 bg-[#111] border border-white/10 text-white/50 hover:text-white hover:bg-white/5 rounded-2xl flex items-center justify-center font-bold transition-all text-sm uppercase tracking-widest"
          >
            Cancel
          </Link>
        </div>
