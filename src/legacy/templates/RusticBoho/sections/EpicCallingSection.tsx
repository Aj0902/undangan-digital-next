...
export default function EpicCallingSection({ data }: { data: Client }) {
  ...
  return (
    <section ref={containerRef} className="relative w-full py-40 px-8 md:px-16 overflow-hidden border-b border-stone-100 flex flex-col items-center">
      {/* Floating Parallax Ornaments - Accurate Placement */}
      <motion.img 
        src="/assets/rustic-boho/images/Or-kiri.svg"
        style={{ y: yOrnLeft, rotate: -25 }}
        className="absolute -left-20 top-20 w-80 md:w-[45rem] opacity-[0.15] pointer-events-none z-0"
        alt="Ornament Kiri"
      />
      <motion.img 
        src="/assets/rustic-boho/images/Or-kanansvg.svg"
        style={{ y: yOrnRight, rotate: 15 }}
        className="absolute -right-20 bottom-10 w-80 md:w-[45rem] opacity-[0.15] pointer-events-none z-0"
        alt="Ornament Kanan"
      />

      {/* Bridge Ornament to Prologue */}
      <motion.img 
        src="/assets/rustic-boho/images/or-bawah-tengah.svg"
        style={{ y: yOrnRight }}
        className="absolute -bottom-24 left-1/2 -translate-x-1/2 w-80 opacity-[0.2] pointer-events-none z-20"
        alt="Bridge Ornament"
      />

      <div className="grid grid-cols-1 md:grid-cols-2 gap-20 md:gap-32 items-center max-w-6xl relative z-10">
        ...
        >
          ...
          <h2 className="font-heading text-fluid-h2 text-stone-900 tracking-tighter leading-[0.85] mb-12 ">
            Pernikahan <br />{" "}
            <span className="font-accent text-[0.8em] text-stone-300 align-middle">Terindah</span>{" "}
            <br /> Kami
          </h2>
